...
class DecisionTreeRegressor:
    ...
    def _split(self, X: np.array, y: np.array, feature: int) -> float:
    ...
    def _best_split(self, X: np.array, y: np.array) -> tuple[int, float]:
        ...
        for i in range(X.shape[1]):
            current_threshold = self._split(X, y, i)
    ...
    def _split_node(self, X: np.ndarray, y: np.ndarray, depth: int = 0,
        ...
        if depth >= self.max_depth or X.shape[0] <= self.min_samples_split:
            return Node(best_f, best_thr, X.shape[0], y.mean(), self._mse(y),
        ...
        return Node(best_f, best_thr, X.shape[0], y.mean(), self._mse(y),
                    ...
                    self._split_node(X=X_r, y=y_r, depth=depth+1, best_f=best_f,
                                     best_thr=best_thr))
